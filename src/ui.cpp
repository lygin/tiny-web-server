    ...
    ui(new Ui::MainWindow)
{
    ...
    //ui->showTableWidget->horizontalHeader()->setSectionResizeMode(QHeaderView::Stretch);
    ui->showTableWidget->setEditTriggers(QAbstractItemView::NoEditTriggers);//禁止编辑
    QStringList tableHeaders;//设置列名
    tableHeaders<<"Date"<<"Time"<<"IP"<<"Port"<<"Method"<<"Url"<<"Respond";
...
void MainWindow::on_startPushBotton_clicked()
{
    ...
    this->lsThread=new ListenThread;
    this->lsThread->setListenSocket(this->listenSocket);
    this->lsThread->setMainPath(ui->pathLineEdit->text().toStdString());
    this->lsThread->start();

    //接收线程sdThread发来的信号，更新UI
    connect(this->lsThread,&ListenThread::acceptOK,[=](QStringList msg){
        ...
        {
            tempStr=msg.at(i);
            if(tempStr == "10053") tempStr = QString::fromStdString(Config::OK_STATUS_STR);
            //qDebug()<<QString::fromStdString(tab[i])<<":"<<tempStr;
...
{
    if(this->lsThread!=nullptr)
    {
        delete this->lsThread;
        this->lsThread=nullptr;
    }
}
...
void MainWindow::on_endPushBotton_clicked()
{
    ...
    QMessageBox::about(this,"提示","关闭服务完成");
    ui->startPushBotton->setEnabled(true);
    ui->pathPushBotton->setEnabled(true);
}
...
{
    QMessageBox::about(this,"author","xiao jiao 1802 - li jun");
}
